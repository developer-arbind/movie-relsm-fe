...
//   socket.on("is:already:talked", (user2: socketUser) =>
//     handlers.removeUser(user2)
//   );
//   socket.on("send:text", ({ text, roomId }: textPayload) =>
//     handlers.sendMessage(text, roomId)
//   );
//   socket.on("user:disconnect", (user2: socketUser) =>
//     handlers.leaveTheRoom(user2)
//   );
//   socket.on("wait:on:queue", (interest: Array<String> | string) =>
//     handlers.findUser(interest)
//   );
...
//   socket.on("track:ready", (user2: socketUser) =>
//     io.to(user2).emit("send:track:to:user2")
//   );
